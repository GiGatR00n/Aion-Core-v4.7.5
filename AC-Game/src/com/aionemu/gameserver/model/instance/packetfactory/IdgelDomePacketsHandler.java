 ...
 */
public class IdgelDomePacketsHandler {

	private final FastList<Future<?>> idgelTask = FastList.newInstance();
	
	private WorldMapInstance instance;
	private IdgelDomeReward idr;
	
	private Integer mapId;
	private int instanceId;
	
	/**
	 * Determines whether the instance is destroying?
	 * it used to avoids spawn of NPCs on InstanceDestroy Event
	 */
	private boolean isInstanceDestroyed = false;
	
	
	public IdgelDomePacketsHandler(Integer mapId, int instanceId, WorldMapInstance instance, IdgelDomeReward idr) {
		this.mapId = mapId;
	...
	/**
	 * Send Requirement Packets For every player entered Instance
	 * <br><br>
	...
	public void sendPreparingPacket(final Player player) {
    	instance.doOnAllPlayers(new Visitor<Player>() {
            ...
            public void visit(Player GroupMember) {
            	PacketSendUtility.sendPacket(GroupMember, new SM_INSTANCE_SCORE(6, idr.getRemainingTime(), idr, instance.getPlayersInside()));
            	PacketSendUtility.sendPacket(GroupMember, new SM_INSTANCE_SCORE(3, idr.getRemainingTime(), idr, player.getObjectId(), 0, player.getRace().getRaceId()));
            	PacketSendUtility.sendPacket(GroupMember, new SM_INSTANCE_SCORE(7, idr.getRemainingTime(), idr, instance.getPlayersInside()));
            }
        });
	}
	
    /**
     * Send 1-Time when Start_Progress and End_Progress
     * <br><br>
     * Start_Progress = 0<br>
     * End_Progress = 2<br>
     */
	public void sendScoreTypePacket() 
    {
    	instance.doOnAllPlayers(new Visitor<Player>() {
            ...
            public void visit(Player player) {
            	PacketSendUtility.sendPacket(player, new SM_INSTANCE_SCORE(2, idr.getRemainingTime(), idr, player.getObjectId()));
            	PacketSendUtility.sendPacket(player, new SM_INSTANCE_SCORE(7, idr.getRemainingTime(), idr, instance.getPlayersInside()));
            }
        });
    }
    
    /**
     * Send when Boss has been Killed or Timeout reached
     * <br><br>
     * OnBossKilled()<br>
     * OnTimeOut()<br>
     */
	public void sendRewardPacket() 
    {
    	instance.doOnAllPlayers(new Visitor<Player>() {
            @Override
            public void visit(Player player) {
            	PacketSendUtility.sendPacket(player, new SM_INSTANCE_SCORE(6, idr.getRemainingTime(), idr, instance.getPlayersInside()));
            	PacketSendUtility.sendPacket(player, new SM_INSTANCE_SCORE(7, idr.getRemainingTime(), idr, instance.getPlayersInside()));
            	PacketSendUtility.sendPacket(player, new SM_INSTANCE_SCORE(5, idr.getRemainingTime(), idr, player.getObjectId()));
            }
        });
    }
    ...
    /**
     * Send only when a NPC has been Killed
     * <br><br>
     * OnNpcDie()
     */
	public void sendNpcScorePacket(final Player player) 
    {
    	instance.doOnAllPlayers(new Visitor<Player>() {
            @Override
            public void visit(Player GroupMember) {
            	PacketSendUtility.sendPacket(GroupMember, new SM_INSTANCE_SCORE(11, idr.getRemainingTime(), idr, player.getObjectId()));
            }
        });
    }
	
    /**
     * Send only when Player <b>Disconnects</b> or <b>Leave</b> Instance
     * <br><br>
     * OnLeaveInstance()
     */
	public void sendPlayerLeavePacket(final Player player) 
    {
    	instance.doOnAllPlayers(new Visitor<Player>() {
            @Override
            public void visit(Player GroupMember) {
            	PacketSendUtility.sendPacket(GroupMember, new SM_INSTANCE_SCORE(8, idr.getRemainingTime(), idr, player.getObjectId()));
            }
        });
    }
	...
	public void sendPlayerDiePacket(final Player player) 
    {
    	instance.doOnAllPlayers(new Visitor<Player>() {
            @Override
            public void visit(Player GroupMember) {
            	PacketSendUtility.sendPacket(GroupMember, new SM_INSTANCE_SCORE(4, idr.getRemainingTime(), idr, player.getObjectId(), 60, player.getRace().getRaceId()));
            	PacketSendUtility.sendPacket(GroupMember, new SM_INSTANCE_SCORE(3, idr.getRemainingTime(), idr, player.getObjectId(), 60, player.getRace().getRaceId()));
            	PacketSendUtility.sendPacket(GroupMember, new SM_INSTANCE_SCORE(7, idr.getRemainingTime(), idr, instance.getPlayersInside()));
            }
        });
    }
	...
	public void sendPlayerRevivedPacket(final Player player) 
    {
    	instance.doOnAllPlayers(new Visitor<Player>() {
            @Override
            public void visit(Player GroupMember) {
            	PacketSendUtility.sendPacket(GroupMember, new SM_INSTANCE_SCORE(4, idr.getRemainingTime(), idr, player.getObjectId(), 0, player.getRace().getRaceId()));
            }
        });
    }
    ...
    public void startInstanceTask()
    {
		...
		idgelTask.add(ThreadPoolManager.getInstance().schedule(new Runnable() {
            ...
            public void run() {
            	SpawnFlameVents();
                openFirstDoors();
                idr.setInstanceScoreType(InstanceScoreType.START_PROGRESS);
                sendScoreTypePacket();//Send 1-Time when Start_Progress and End_Progress
            }
        }, idr.getPreparingTime()));
    ...
    protected void SpawnUnstableIdeEnergies() 
	{
    	 ...
    	 */
    	float Rx = (float) Rnd.get(-2, 2);
    	float Ry = (float) Rnd.get(-2, 2);
    ...
    protected void sp(final int npcId, 
		    		  final float x, 
		    		  final float y, 
		    		  final float z, 
		    		  final byte h, 
    ...
    protected VisibleObject spawn(int npcId, 
					    		  float x, 
					    		  float y, 
					    		  float z, 
					    		  byte heading) 
