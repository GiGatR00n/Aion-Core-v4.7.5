...
public class MySQL5BrokerDAO extends BrokerDAO {
    ...
    private boolean updateBrokerItem(final BrokerItem item) {
        boolean result = DB
                .insertUpdate(
                        "UPDATE broker SET `is_sold` = ?, `is_settled` = 1, `settle_time` = ?, `is_splitsell` = ? WHERE `item_pointer` = ? AND `expire_time` = ? AND `seller_id` = ? AND `is_settled` = 0",
                        new IUStH() {
                            @Override
                            public void handleInsertUpdate(PreparedStatement stmt) throws SQLException {
                                stmt.setBoolean(1, item.isSold());
                                stmt.setTimestamp(2, item.getSettleTime());
								stmt.setBoolean(3, item.isSplitSell());
                                stmt.setInt(4, item.getItemUniqueId());
                                stmt.setTimestamp(5, item.getExpireTime());
