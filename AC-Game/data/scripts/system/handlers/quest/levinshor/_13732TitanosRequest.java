 ...
 * @author pralinka
 */
public class _13732TitanosRequest extends QuestHandler {

	private final static int questId = 13732;

	public _13732TitanosRequest() {
		super(questId);
	}
	...
	public boolean onKillInWorldEvent(QuestEnv env) {
		...
		if (env.getVisibleObject() instanceof Player && player != null && player.isInsideZone(ZoneName.get("EAST_RELAY_600100000"))) {
	...
	public boolean onEnterZoneEvent(QuestEnv env, ZoneName zoneName) {
		if (zoneName == ZoneName.get("EAST_RELAY_600100000")) {
			Player player = env.getPlayer();
			if (player == null)
				return false;
			QuestState qs = player.getQuestStateList().getQuestState(questId);
