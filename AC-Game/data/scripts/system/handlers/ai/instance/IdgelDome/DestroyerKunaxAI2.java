...
//				    			Point randPoint = MathUtil.get2DPointOnCircle(new Point((int) 264.4382f, (int)258.58527f), new Point((int) getOwner().getX(), (int)getOwner().getY()), 16);
    ...
    private Player getRndTargetInBossCircle(float CircleRadius) {
        ...
        for (Player player : getKnownList().getKnownPlayers().values()) {
            if (!PlayerActions.isAlreadyDead(player) && isInBossCircle(player, CircleRadius)) {
                players.add(player);
            }
        }
        if (players.isEmpty()) {
    ...
    public AttackIntention chooseAttackIntention() {
    	...
    	/*
    	 * Specifies whether the Target is in Ground or Air?
    	 * 
    	 * The Z-axis difference between Boss and Player is "ToleranceZ"
    	 * 0 = Ground Attacks
    	 * 1 = Air Attacks
    	 * (v4.7.5.17)
    	 */
